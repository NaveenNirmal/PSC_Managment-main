...
public class IssuePaddyController {
            ...
            if (issuePaddyDate == 1) {
                ...
                if (issuePaddy.getQuantity() > issuePaddy.getVehicle().getCapacity()) {
                    message.addFlashAttribute("error",
                            "Error : The selected vehicle, cannot hold the specific quantity");
                    return "redirect:/admin/issuePaddy/create";
                } else {
                    // Generate Revenue Of Sale
                    float revenue = generateRevenue(issuePaddy);
                    issuePaddy.setRevenue(revenue);
                    issuePaddy.setStatus("Request");
                    issuePaddyService.save(issuePaddy);
                    message.addFlashAttribute("success",
                            "Success : The Issue Paddy Request has been saved successfully");
    ...
    public float generateRevenue(IssuePaddy issuePaddy) {
        ...
        Float branchCurrentIncome = issuePaddy.getBranch().getIncome() == null ? 0 : issuePaddy.getBranch().getIncome();
        float income = 0;
        if (branchCurrentIncome == 0) {
            income += revenue;
        } else {
